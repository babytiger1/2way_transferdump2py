...
class XgboostTreeNode:
  def __init__(self,nodeNum=-1,splitCond=None,leftChild=None,rightChild=None,isLeaf=True,leafValue=None,cover=None,gain=None,caseMissingChild=None):
    self.__filename = "output_res.txt"
  ...
  def parseLine(self,line):
    ...
    if 'leaf' in line:
      ...
      splitFeature = 'data['+columnDict[splitFeature] + ']'
      cond = splitFeature + "<" +splitValue
...
class XgboostTree:
  ...
  def printPath(self,i,fout):
    path = [None]*1000
    self.printPathRecur(self.__root,path,0,fout,i)


  def printPathRecur(self,node,path,pathLen,fout,i):
    if node is None:
      return
    path[pathLen] = node.getNodeNum()
    pathLen += 1
    if node.getLeftChild() is None and node.getRightChild() is None:      
      self.printRules(path,pathLen,fout,i)
    else:
      self.printPathRecur( self.__treeNodes[node.getLeftChild()] , path , pathLen ,fout,i)
      self.printPathRecur( self.__treeNodes[node.getRightChild()], path , pathLen ,fout,i)
  


  def printArray(self,ints,len,fout,i):
    #with open(self.__filename,a) as 
    #fout.write( '/*')
    # for i in range(len):
    ...
    fout.write( '/*'+str(i)+'\n')
    #fout.write( '*/\n')
  def printRules(self,ints,len,fout,num,index=True,value= False):
    for i in range(len):
      #print ints[i],
      if i==len-1:
        if index == True:
          fout.write( str(ints[i]) )   #remove "" from leave value 
        if value == True:
...
def generateRtdRule(fin,fout):
  ...
  for line in fin.readlines():
    ...
    if '/*' not in line:
      ...
      fout.write( '    if('+' and '.join(nodes[:-1])+'):'+ '\n' + '             return '+nodes[-1]+'\n' )
    else:
       fout.write('    return 0 \n')
       fout.write( "if i == "+line.split("*")[1].strip() +': \n')

def main(fout,xgb_dumpfile='bst.model.txt'):
  xgboostTrees = parseXgbDumpFile(xgb_dumpfile)
  for i in range(len(xgboostTrees)):
    fout.write( '/*'+str(i)+'\n')
    #fout.write( ('*booster[%d]*/\n')%(i) )
    xgboostTrees[i].printPath(i,fout)
...
columnDict={}
for i,columnname in enumerate(columnName):
    columnDict[columnname] = str(i)
...
if __name__ == '__main__':
  #s = StringIO.StringIO()
  s=open("output_res.txt","w")
  main(fout = s)
  s.seek(0)
  s.close()
  o = open("output_res.txt","r")
  re = open("final_output.txt","w")
  generateRtdRule(o , re )
  o.close()
  re.close()
